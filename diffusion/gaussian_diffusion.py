...
class GaussianDiffusion:
    ...
    def masked_l2(self, a, b, mask):
        ...
        # calculate the count of non-mask elements
        non_zero_elements = sum_flat(mask) * n_entries
        # total loss of each batch / count = average loss of each batch
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            terms["mse"] = self.masked_l2(target, model_output, mask)

            # minimize the gap between conseutive frame
            target_gap = target[:, :, :, 1:] - target[:, :, :, :-1]
            model_output_gap = model_output[:, :, :, 1:] - model_output[:, :, :, :-1]
            terms["gap"] = self.masked_l2(target_gap, model_output_gap, mask[:, :, :, 1:])    
            
            terms["loss"] = terms["mse"] + terms.get('vb', 0.) +\
                                     terms.get('gap', 0.) # L_dis
